...
def get_new_slice_mask(
    current_matched_index, new_matched, new_unmatched, max_cell_num, img_current_slice_shape
):
    ...
    for slice_num in range(1, img.shape[0]):
        ...
        for i in range(len(current_slice_cell_coords)):
            if len(current_slice_cell_coords[i]) != 0:
                new_slice_search_num = np.unique(
                    list(map(lambda x: img_new_slice[tuple(x)], current_slice_cell_coords[i]))
                )
                best_JI = 0
                current_slice_cell_best = []
                for j in new_slice_search_num:
                    if j != 0:
                        if (j - 1 not in new_slice_cell_matched_index_list) and (
                            i not in current_slice_cell_matched_index_list
                        ):
                            current_slice_cell, new_slice_cell, JI = get_matched_cells(
                                current_slice_cell_coords[i], new_slice_cell_coords[j - 1]
                            )
        ...
        new_slice_cell_unmatched_list, new_slice_cell_unmatched_index_list = get_unmatched_list(
            new_slice_cell_matched_index_list, new_slice_cell_coords
        )

        new_slice_updated_mask = get_new_slice_mask(
            current_slice_cell_matched_index_list,
            new_slice_cell_matched_list,
            new_slice_cell_unmatched_list,
            len(np.unique(new_img[:slice_num])),
            img_current_slice.shape,
        )
