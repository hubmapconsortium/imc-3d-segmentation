...
def convert(obj_file: Path, glb_file: Path, mapping: Dict[str, int]):
    ...
    for obj in objs_to_assign:
        obj.name = f"Cell_{mapping[obj.name.removeprefix('Cell_')]}"
...
def main(directory: Path, base_dir: Path = output_path):
    with open(directory / "cell_id_mapping.json") as f:
        mapping = json.load(f)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        raise ValueError(
            "Need exactly one argument: directory containing OBJ files and JSON mapping file"
        )
