...
def match_repair_cell_nucleus(nuclear_slice, cell_slice):
	...
	for i in range(len(cell_coords)):
		if len(cell_coords[i]) != 0:
			...
			nuclear_search_num = np.unique(list(map(lambda x: nuclear_slice[tuple(x)], current_cell_coords)))
...
def match_3D_cells(mask_XY, mask_XZ, mask_YZ, data_dir, output_dir):
	...
	for index in new_coords.index[1:]:
		if len(new_coords[index][0]) < 100:
			...
			mask_binary_boundary = find_boundaries(mask_binary[z_min:z_max, x_min:x_max, y_min:y_max], mode = 'outer')
